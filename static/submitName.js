...
document.addEventListener("DOMContentLoaded", () => {
    ...
    function render() {
        ...
        function draw_point(x, y, connectTo) {
            if (connectTo) {
    ...
    socket.on("leaveRoom", data => {
        ...
        alert(name + " has disconnected to you!");
    });

    socket.on("joinRoom", data => {
        let name = data.username;
        alert(name + " has connected to you!");
    });
    ...
    document.addEventListener('click', event => {
        ...
        if (Info.id === "color set") {
            const color = Info.value;
        ...
        if (Info.id === "needClick") {
            ...
            roomNow = userConnNow;
            Info.textContent = "discon";
            Info.id = "disconnect";
            Info.style = "background: #f2cb0a";
